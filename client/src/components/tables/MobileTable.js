	...
	return (
		<div className="mobile-table-single">
		  <div className="mb-2 py-2">
		  	<SelectInput options={headerOptions} value={header} handler={headerHandler}/>
		  </div>
				...
				<tbody>
					{
						subCategories.map((subCategory, s) => {
							...
							return (
								<tr key={`table-row-${s}`}>
								  <th>{subCategory}</th>
								  <td key={`cell-${s}`}>{ element[0] == undefined ? 'N/A' : element[0]  }</td>
								</tr>
							)
						})
					}
				...
				<tbody>
					{
						rowsArray.map((row, r) => {
							return (
								<tr key={`table-row-${r}`}>
									<td key={`cell-${header}`}>
									  {dataTable[header][r] == undefined ? 'N/A' : dataTable[header][r]}
									</td>
								</tr>
							)
						})
...
const MobileTable = (props) => {
  ...
  const headerOptions = createTableOptions(headers);
	...
	return (
		...
		{
			type == 'single' ? <SingleCategory  data={data} options={options} /> : <DoubleCategory options={options} data={data} />
		}
		</div>
	)
}
