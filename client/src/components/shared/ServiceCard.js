...
const CardSwitch = (props) => {
	...
	switch(card.type){
	  case 'acitve':
	  	return (<Link to={card.path} className={classes}>{children}</Link>);
	  case 'inactive':
	  	return (<div to={card.path} className={classes}>{children}</div>);
	  case 'external':
	  	return (<Link to={{pathname: card.path}} className={classes} target="_blank">{children}</Link>);
	  default:
	  	return (<Link to={card.path} className={classes}>{children}</Link>);
	}
}
			...
			<div className="p-2 service-card">
				<div className="m-1 pb-2 service-card-inner">
					<div className="image-container">
						<img src={card.image.src} alt={card.image.alt} className={card.image.preprocess ? "preprocess-image" : ""} />
					</div>
