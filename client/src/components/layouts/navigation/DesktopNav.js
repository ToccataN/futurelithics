  ...
  return (
    ...
    >
      <DropdownToggle
        toggle={toggle}
        caret={false}
        className="desktop-toggle-sub"
      >
        <div className="d-flex justify-content-between align-items-center">
          <FontAwesomeIcon icon={faArrowLeft} className=" align-middle icon" />
          <Link to={route.path} className="flex-grow text-end w-100 pe-2">
            {route.title}
          </Link>
        </div>
      </DropdownToggle>
            ...
            if (filteredRoutes.length > 0) {
              ...
              return (
                <Link
                  ...
                  className="flex-grow text-end mw-100 sub-item-link"
                >
                  <DropdownItem>{route.title}</DropdownItem>
                </Link>
              );
            }
          })}
    ...
    <div className="d-none d-md-block">
      ...
      >
        <DropdownToggle
          toggle={toggle}
          caret={false}
          className={`desktop-toggle ${open ? "focused" : ""}`}
        >
              ...
              if (routeFilter(route).length > 0) {
                ...
                return (
                  <Link
                    to={{ pathname: route.path }}
                    key={route.name}
                    className="flex-grow text-end mw-100 item-link"
                  >
                    <DropdownItem className="p-2">{route.title}</DropdownItem>
                  </Link>
                );
              }
            })}
